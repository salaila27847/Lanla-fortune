...
- **การจั่วไพ่ทาโรต์/ออราเคิล**: **แก้ไข 2026-08-16** — เปลี่ยนจากสุ่มไพ่ฝั่ง server *หลังจาก*
  ผู้ใช้แตะเลือกครบแล้ว (ตำแหน่งที่แตะบนจอเป็นแค่ animation cosmetic ไม่มีผลต่อไพ่ที่ได้จริง) มาเป็น
  **ให้ผู้ใช้เลือกไพ่ของตัวเองจริง** — ผู้ใช้ตัดสินใจว่าซื่อสัตย์กับผู้ใช้มากกว่า เพราะถึงยังไงผู้ใช้ก็
  เห็นผลหลังกดทำนายอยู่ดี ไม่มีประโยชน์ที่จะหลอกด้วยตำแหน่งปลอม ครอบคลุมทั้งไพ่ทาโรต์และออราเคิล
  ทุกจุด (รวม follow-up "ถามเพิ่ม") — สถาปัตยกรรมใหม่: client เรียก `POST /api/oracle/draw` หรือ
  `POST /api/tarot/draw` ก่อน เพื่อรับสำรับทั้งชุดที่สับแล้ว (พร้อมเนื้อหาไพ่จริงทุกใบ, ทาโรต์มี
  orientation ตัดสินไว้ล่วงหน้าด้วย) ตำแหน่งที่ผู้ใช้แตะบนจอจึงแม็ปกับไพ่จริงที่ตัดสินไว้แล้ว จากนั้น
  client ส่งไพ่ที่แตะเปิดจริง (card id ตามลำดับที่แตะ) กลับไปที่ `POST /api/reading` — **ตัดสินใจ
  "trust client"**: ไม่มี token/session ฝั่ง server กันการปลอมแปลงว่าจั่วอะไรมาจริง (เหตุผล: ไม่มี
  คู่แข่ง/เดิมพันในระบบนี้ ผู้ใช้ปลอมไพ่ของตัวเองก็แค่หลอกตัวเอง) แต่เนื้อหา (ความหมาย/keywords) ยังคง
  authoritative จาก knowledge base เสมอ — client ส่งแค่ id ไม่ใช่เนื้อหา backend lookup ความหมายจริง
  เองทุกครั้งผ่าน `build_result_from_picks()` ดูรายละเอียด schema ที่ `docs/data-schema.md` หัวข้อ
  "Draw-before-reveal"
