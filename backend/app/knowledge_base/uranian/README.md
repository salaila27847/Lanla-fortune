...
witte_pictures.yaml        ← glossary ละเอียดกว่า axis_meanings.yaml อีกชั้น — คู่ปัจจัย + ปัจจัย
                            ...
                            ผู้ใช้ส่งมาให้ (OCR คุณภาพต่ำ ต้องอ่านสร้างใหม่ทุกรายการ) — ตอนนี้มี
                            หมวด Meridian ครบ (M + อีก 21 ปัจจัย, 412 รายการ) และหมวด Aries ครบ
                            (ARIES + อีก 20 ปัจจัย — ไม่รวม M เพราะอยู่ในหมวด Meridian แล้ว, 386
                            รายการ) รวม 798 รายการ ยังไม่ได้ทำอีก 20 หมวดที่เหลือของหนังสือ (Sun,
...
4. **Planetary-picture findings** — หา midpoint structure บน 90° dial ระหว่างปัจจัยทั้งหมด:
   ...
   1. **Type I ที่ตรงกับ `witte_pictures.yaml` เป๊ะๆ** (คู่ปัจจัย + ปัจจัยที่ 3 ตรงกัน) — เจาะจงและ
      authoritative ที่สุด (weight 0.95) ตอนนี้ครอบคลุมหมวด Meridian (M-pair) และ Aries (ARIES-pair)
...
- `planetary_pictures.yaml` เป็นการคัดสรร 50 คู่ ไม่ใช่ชุดสมบูรณ์ตามต้นฉบับ (แต่ `witte_pictures.yaml`
  ครอบคลุมได้ละเอียดกว่ามากสำหรับหมวด Meridian/Aries แล้ว — ดูด้านบน)
...
- `witte_pictures.yaml` มีแค่หมวด Meridian + Aries (41 คู่จากทั้งหมด ~231 คู่ที่เป็นไปได้ในหนังสือ)
  — หมวด Sun/Ascendant/Moon/Node และดาวเคราะห์/TNP อีก 16 ดวงยังไม่ได้ทำ (ตำราต้นฉบับผู้ใช้ส่งมา
  เป็นไฟล์ scan+OCR ทั้งเล่ม ~255 หน้า — เก็บสำเนาไว้ในเซสชันที่แปลได้เท่านั้น ยังไม่ได้ก็อปปี้เข้า
  repo เพราะไฟล์ใหญ่มาก 9,599 บรรทัด — ต้องขอผู้ใช้ส่งซ้ำถ้าจะทำหมวดถัดไปในเซสชันใหม่) บาง base
  pair ในหมวด Meridian/Aries เองก็ขาดบางรายการเพราะ OCR กู้คืนไม่ได้ (ดู comment `# missing:` ใน
  ตัวไฟล์ yaml) — หมวด Aries คุณภาพ OCR แย่กว่าหมวด Meridian อย่างชัดเจน โดยเฉพาะคู่ ARIES+HADES
  ที่กู้คืนได้แค่ ~17/20 รายการ
